...
- **Качество детекции**: YOLOv11m показывает высокий recall в плотных сценах — детектит 8–12 людей на кадр в среднем, с confidence >0.7 для большинства. False positives (на картинах/постерах) минимальны благодаря фильтру правой зоны и динамическому min_size. Полупрозрачные боксы не перекрывают кадр (>80% видимости), объекты различимы.
- **Метрики**: Средний confidence ~0.66 (из тестов), std ~0.15, min ~0.35, max ~0.95. Общее детекций ~5000–6000. В сложных частях (толпа >10 человек) recall ~80–85%, precision ~85–90% (гипотетически, по визуальному сравнению с ground truth на скринах).
- **Гистограмма**: Гистограмма показывает пик в 0.7–0.9, что указывает на уверенность; график по кадрам — стабильные колебания без резких провалов.
